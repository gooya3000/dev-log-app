...
## LLM 교체 시 동반 업데이트 체크리스트
LLM 공급자(Gemini → 다른 모델, 또는 그 반대) 또는 모델 ID 를 교체하는 변경에서는 코드뿐 아니라 아래 지시 문서들이 동시에 갱신돼야 함을 메인 세션에 보고한다 (직접 수정 금지 — 지시 문서 자가 수정 금지 규칙).
- 본 에이전트 정의 `description` 의 어댑터/모델명
- `docs/PLAN.md` §3 의 ai 패키지 구조 주석에 적힌 클래스명·모델명
- `docs/HANDOFF.md` 의 "최근 결정" 또는 어댑터 상태 라인
- README 가 존재한다면 README 의 환경변수·키 발급 안내
- 보안 테스트의 fake 키 예시(공급자별 접두어 컨벤션이 다르면 갱신)
